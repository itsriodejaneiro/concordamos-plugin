		...
		<a href={panel ? getPanelUrl(voting.permalink) : voting.permalink} className={classNames('voting-card', voting.time)}>
			<article >
				...
				<main>
					...
					<p>{voting.content}</p>
					{ panel ? null : (
						<p className="voting-card__status"><Image src={requiresLogin ? 'private.svg' : 'public.svg'}/> {requiresLogin ? (isOpen ? __('Login required', 'concordamos') : __('Login required to see results', 'concordamos')) : __('No login required', 'concordamos')}</p>
					)}
